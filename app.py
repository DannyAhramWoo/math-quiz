...
def natural_sort_key(s):
    """파일명을 숫자 순서로 정렬 (01, 02, ... 10, 11)"""
    return [int(c) if c.isdigit() else c.lower() for c in re.split(r'(\d+)', s)]


def get_category(filename):
    """파일명에서 카테고리 추출: 'AP 2021_01.png' → 'AP 2021'"""
...
def list_problems():
    """problems/ 폴더를 카테고리별로 그룹화해서 반환 (서브폴더 및 루트 파일 모두 지원)"""
    categories = {}
...
def update_progress():
    ...
    if correct:
        ...
        progress[name]["mastered"] = True   # 1번 맞으면 바로 제외
    else:
